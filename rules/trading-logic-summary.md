...
| 位置系 | 3 | BB下限付近 | %B < 30 |
| 勢い系 | - | MACDフレッシュモメンタム | MACD > Signal AND (フレッシュクロス OR ヒストグラム増加) |

**勢い系の詳細:**
- **前提条件**: MACD > Signal（クロス済み）
- **追加条件（どちらか一方）**:
  - フレッシュクロス: 直近5本以内にヒストグラムがゼロクロス（負→正）
  - ヒストグラム増加: hist[-1] > hist[-2] > hist[-3]（3本連続増加）

**設計根拠:**
- 単なる「MACD > Signal」だと、10本前にクロスして勢いが衰えている場合も発火してしまう
- フレッシュクロス（5本=5時間）で直近のクロスを検出し、タイムリーなエントリーを確保
- ヒストグラム増加でクロス後の加速局面もカバー
- MACD > Signal を前提条件として維持し、デッドキャットバウンスを回避
...
| 位置系 | 3 | BB上限付近 | %B > 70 |
| 勢い系 | - | MACDフレッシュモメンタム | MACD < Signal AND (フレッシュクロス OR ヒストグラム減少) |

**勢い系の詳細:**
- **前提条件**: MACD < Signal（クロス済み）
- **追加条件（どちらか一方）**:
  - フレッシュクロス: 直近5本以内にヒストグラムがゼロクロス（正→負）
  - ヒストグラム減少: hist[-1] < hist[-2] < hist[-3]（3本連続減少）
