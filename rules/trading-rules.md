...
**なぜ「SMA20傾き」から変更したか：**
- 旧条件（SMA20 slope）はモメンタムの方向を見ていたが、押し目（価格がSMA20に接近）を検出できなかった
- 押し目買い戦略の本質は「上昇トレンド中にSMA20まで下落した場面」を捉えること
- 新条件は「価格がSMA20の±1%バンド内にある」ことで押し目/戻りを検出

**なぜ±1%のバンド幅か：**
- ±1%は多くの通貨ペアで適度な範囲（USD/JPYなら約±1.5円）
- 固定値で開始し、実運用ログを見て通貨別調整やATRベースへの移行を検討

**Buy/Sell共通条件の理由：**
- 価格がSMA20付近にあることは方向性を示さない
- 方向の判断はMTFフィルター（SMA20 > SMA50 → Buy許可、SMA20 < SMA50 → Sell許可）で制御
- 他条件（RSI、MACD、BB）も方向を示すため、この条件は「位置」のみを担当
