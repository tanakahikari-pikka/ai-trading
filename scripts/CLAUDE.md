...
**Buy発火条件: (位置系 >= 2) AND (勢い系 >= 1)**

| カテゴリ | 条件 |
|----------|------|
| 位置系 | RSI < RSI_BUY_THRESHOLD（動的） |
| 位置系 | SMA20 - ATR < 価格 <= SMA20（押し目） |
| 位置系 | %B < 30（BB下限付近） |
| 勢い系A | MACD > Signal（クロス済み） |
| 勢い系B | ヒストグラム3本連続増加（勢い回復中） |

**Sell発火条件: (位置系 >= 2) AND (勢い系 >= 1)**

| カテゴリ | 条件 |
|----------|------|
| 位置系 | RSI > RSI_SELL_THRESHOLD（動的） |
| 位置系 | SMA20 < 価格 < SMA20 + ATR（戻り） |
| 位置系 | %B > 70（BB上限付近） |
| 勢い系A | MACD < Signal（クロス済み） |
| 勢い系B | ヒストグラム3本連続減少（勢い減退中） |
