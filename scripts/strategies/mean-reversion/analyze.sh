#!/bin/bash
# Mean Reversion Strategy - Technical Analysis
...
#
# Strategy Logic:
#   Buy: RSI oversold + Price near BB lower band (position indicators)
#        + MACD bullish momentum (optional, adds confidence)
#   Sell: RSI overbought + Price near BB upper band (position indicators)
#         + MACD bearish momentum (optional, adds confidence)
...
echo "=== Technical Analysis (Mean Reversion) ===" >&2
echo "Current Price: $CURRENT_PRICE" >&2
echo "" >&2

# Calculate all indicators
RSI_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/rsi.sh" 14 2>/dev/null || echo '{"rsi":50}')
SMA20_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/sma.sh" 20 2>/dev/null || echo '{"sma":0}')
SMA50_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/sma.sh" 50 2>/dev/null || echo '{"sma":0}')
EMA12_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/ema.sh" 12 2>/dev/null || echo '{"ema":0}')
EMA26_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/ema.sh" 26 2>/dev/null || echo '{"ema":0}')
MACD_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/macd.sh" 12 26 9 2>/dev/null || echo '{"macd":0,"signal":0,"histogram":0}')
BB_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/bollinger.sh" 20 2 2>/dev/null || echo '{"upper":0,"middle":0,"lower":0,"position":"unknown","percent_b":50}')
ATR_DATA=$(cat "$TEMP_FILE" | "$INDICATORS_DIR/atr.sh" 14 2>/dev/null || echo '{"atr":0,"atr_percent":0,"volatility":"unknown"}')
...
# Extract %B from Bollinger Bands
...
# Fresh cross detection for sell
...
# ATR filter: suppress signals in abnormal volatility
...
echo "SMA20 Slope: up=$SMA20_SLOPE_UP down=$SMA20_SLOPE_DOWN" >&2
